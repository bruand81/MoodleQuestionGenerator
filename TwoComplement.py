...
class BinaryToDecimal(mxqg.Question):
    bit_number = 8

    def question_generate(self, _quiz_number=0):
        quiz = mxqg.Quiz(name='Da binario a decimale', quiz_number=_quiz_number, lang='it')
        ...
        correct_answer = f"{BinaryConversion.binary_to_decimal(quiz.data[0])}"
    ...
    def incorrect_answers_generate(self, quiz, size=4):
        ...
        while len(answers) < size:
            incorrect_ans = f"{random.randint(pow(2, self.bit_number), pow(2, self.bit_number+1)-1)}"
            ans['data'] = f"{incorrect_ans}<sub>10</sub>"
            ans[
                'feedback'] = 'No, risposta sbagliata '
    ...
    def question_text(self, quiz):
        return f'Indicare quale fra i seguenti numeri rappresentati in <u>sistema decimale</u> ' \
               f'corrisponde al numero {quiz.data[0]}<sub>2</sub> espresso in <u>sistema binario semplice</u>'

    def answer_text(self, ans):
        return ans['data']


class DecimalToBinary(mxqg.Question):
    bit_number = 8

    def question_generate(self, _quiz_number=0):
        quiz = mxqg.Quiz(name='Da decimale a binario', quiz_number=_quiz_number, lang='it')
        # generates a quiz data
        target_value = random.randint(pow(2, self.bit_number), pow(2, self.bit_number+1)-1)
        quiz.data = [f"{target_value}"]
        quiz.quiz_identifier = hash(quiz.data[0])
        correct_answer = BinaryConversion.decimal_to_binary(target_value)
        quiz.data.append(correct_answer)
        return quiz

    def correct_answers_generate(self, quiz, size=1):
        answers = []
        ans = {'fraction': 100, 'data': f"{quiz.data[1]}<sub>2</sub>", 'feedback': 'Ottimo!'}
        # quiz.answers.append(ans)
        answers.append(ans)
        answers = mxqg_common.answer_union(answers)
        return answers
    ...
    def incorrect_answers_generate(self, quiz, size=4):
        ...
        ans = {'fraction': 0, 'data': 0}
        while len(answers) < size:
            generated_value = random.randint(pow(2, self.bit_number), pow(2, self.bit_number+1)-1)
            ans['data'] = f"{BinaryConversion.decimal_to_binary(generated_value)}<sub>2</sub>"
            ans['feedback'] = 'No, risposta sbagliata '
    ...
    def question_text(self, quiz):
        return f"Indicare quale fra i seguenti numeri rappresentati in " \
               f"<u>sistema binario semplice</u> " \
